...
let handler = async (m, { conn, command, usedPrefix, text }) => {
    ...
    if (!text) return conn.sendButton(m.chat, `Uhm.. teksnya mana?\n\nContoh:\n${usedPrefix + command} tes`, '© Ramzofc', 'Daftar Pesan', `${usedPrefix}list${which}`, m)
    let msgs = db.data.msgs
    if (text in msgs) return conn.sendButton(m.chat, `'${text}' telah terdaftar, gunakan nama lain!`, '© Ramzofc', 'Daftar Pesan', `${usedPrefix}list${which}`, m)
...
Jika Getmsg diaktifkan maka tidak perlu lagi mengetik *${usedPrefix}get${which}*`, '© Ramzofc', 'Aktifkan', '.1 getmsg', m)
}
